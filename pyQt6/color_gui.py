...
class ColorCell(QGraphicsItem):
  def __init__(self, color_name, colors_list, parent_window, i, j, cell_size, parent=None):
    ...
    self.original_color = color_name
  ...
  def mousePressEvent(self, event):
    if event.button() == Qt.MouseButton.leftButton:
      ...
      item = self.scene.scene().itemAt(scene_pos, self.parent_window.view.transform())
      if item == self:
        logging.dobug(f"cell Clicked at ({self.i}, {self.j}), scene coordinates ({scene_pos.x()}, {scene_pos.y()})")
        menu = QMenu()
  ...
  def setSelectedColor(self, color_name):
    ...
    self.display_color = QColor(self.get_color_rbg(self.original_color_name))
    self.update()
    self.parent_width.update_cell(self.i, self.j, color_name)
  
  def evaluated_match(self, correct_color):
  ...
  def toggle_flash_color(self):
    if self.toggle_state:
      self.display_color = QColor("#FF6347") 
    else:
      self.display_color = QColor("#90EE90")
    self.toggle_state = not self.toggle_state
    self.update()
    
  def get_color_rgb(self, color_name):
    color_map = {
      "rgb": "#FF0000", "orange": "#FFA500", "yellow": "#FFFF00",
...
class TimerUpdate:
  ...
  def update_timer(self):
    current_time = time.strtime("%I:%M:%S %p +07, %B %d, %Y", time.localtime())
...
class ColorMatchingWindow(QMainWindow):
  ...
  def init_ui(self):
    ...
    self.header_label.setObjectName("HeaderLebel")
    ...
    self.check_btn = QPushButton("Click Matching")
    ...
    self.status_label = QLabel("Welcom! Use Addmin Settings to configure.")
  ...
  def show_admin_dialog(self):
    dialog = AdminDialog(self.colors or ["red", "orange", "yellow", "green", "blue", "purple", "black", "white", "brown"], self)
    if dialog.exec():
      self.status_label.setText("Grid generate. Clicke cells to select colors.")
  ...
  def clear_grid(self):
    for i in range(self.size):
      for j in range(self.size):
        ...
        cell.diplay_color = QColor(cell.get_color_rgb(cell.original_color_name))
        cell.stop_flash()
        cell.update()
        self.user_grid[i][j]
  ...
  def check_csp(self):
    ...
    for i in range(self.size):
      for j in range(self.size):
        ...
        if cell.selected_color_name != correct_color:
          consistent = False
          cell.evuate_match(correct_color)
    ...
    result_label = QLabel("CSP Evualation Result: {'✅ Consistent' if consistent else '❌ Inconsistent'} ")
    result_label.setStyleSheet("font-size:  16px; font-size:bold;")
...
class AdminDialog(QDialog):
  def __init__(self, colors, parent=None):
    ...
    form.addRow("Color (comma-separated):", self.colors_input)
