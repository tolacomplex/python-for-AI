...
class ColorCell(QGraphicsItem):
    ...
    def toggle_flash_color(self):
        if self.toggle_state:
            self.display_color = QColor("#FF6347")  # red flash
        else:
            self.display_color = QColor("#90EE90")  # green flash
...
class ColorMatchingWindow(QMainWindow):
    ...
    def generate_game(self):
        ...
        spacing = 10
        cell_size = (available_size - spacing * (self.size + 1)) / self.size

        x_offset = (view_width - (self.size * (cell_size + spacing))) / 2 + cell_size / 2
        y_offset = (view_height - (self.size * (cell_size + spacing))) / 2 + cell_size / 2
    ...
    def check_csp(self):
        ...
        score = sum(1 for i in range(self.size) for j in range(self.size)
                    if self.user_grid[i][j] == self.init_colors[i][j])
