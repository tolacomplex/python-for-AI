...
try:
  ...
  print("Speech recognition not available. Install with: pip install SpeechRecognition pyaudion")
  
  # Export System Components
  class ExportDialog(QDial):
    ...
    def init_ui(self):
      ...
      self.font_manager.apply_font(data_group)
      ...
      data_group.setLayout(data_group)
      
      # Options
      options_group = QGroupBox("Export Options")
      self.font_manager.apply_font(options_group)
    ...
    def get_export_setting(self):
      """Get selected export setting"""
      formats = []
    ...
    try:
      with open(filename, 'w', newline='', encoding='utf-8') as csv_file:
        if data_type == "dictionary":
          ...
          for word in words:
            row = list(word[10], word[11]) # basic fiels
          ...
          analytics_data = [
            ...
            ("Session_count", user_facts.get("search_count", 0), "Total searhes performed"),
            ("Session_Time", user_facts.get("search_time", 0), "Current session time in seconds"),
  ...
  def _export_json(self, data_type, filename, options):
    ...
    try:
      ...
      if data_type == "dictionary":
        ...
        # Convert defaults to regular dicts for JSON serialization
    ...
    try:
      with open(filename, 'w', encoding='utf-8') as html_file:
        ...
        if data_type == "dictionary":
          ...
          html_file.write("<th>English</th>Khmer</th><th>Definition</th><th>Example</th>\n")
          html_file.write("</tr>\n")
          ...
          html_file.write(f"<p><strong>Total Word:</strong> {len(words)}</p>\n")
  ...
  def run(self):
    ...
    try:
      ...
      try:
        ...
        text = self.receivers.recognize_google(audio)
        self.speech_recognized.emit(text)
      except sr.RequestError:
        try: 
          # Fallback to offline recognition (if available)
          text = self.receivers.recognize_sphinx(audio)
          self.speech_recognized.emit(text)
        except:
          self.speech_error.emit("Could not recognize speech. check internet connectionn.")
      except sr.UnknownValueError:
        self.speech_error.emit("Could not understand audio. Please speak clearly.")
    
    except sr.WaitTimeoutError:
      self.recording_stopped.emit()
      self.speech_error.emit("Listening timeout. No speak detected.")
    except Exception as e:
      self.recording_stopped.emit()
    ...
    try:
      ...
      try:
        ...
        try:
          ...
          self.speech_error.emit("Could not recognition speech. Check internet connection.")
      except sr.UnknownValueError:
        self.speech_error.emit("Could not understand timeout. No speech detected.")
...
class ExpertSystemEngine:
  ...
  def initialize_rules(self):
    ...
    self.add_rule(Rule(
      ...
      ["Try_searching for related objects, places, or things",
       "Consider checking spelling or trying synonyms"]
    ))
    self.add_rule(Rule(
      "verb_suggestions",
      ["word_type == 'verb'", "not_found == True"],
      ["Try searching for actions words or infinitives forms",
       "Check for irregular verb forms"]
    ))
    # User beharvior rules
    self.add_rule(Rule(
      "freguency_searcher",
      ["search_count > 10"],
      ["You're an active learner! Try exploring words categories",
    ...
    self.add_rule(Rule(
      ...
      ["Explore complex grammar partters",
  ...
  def infer(self) -> List[str]:
    """Run inference engine to generate suggestions"""
    suggestions = []
    self.interence_chain.clear()
...
class SimpleWordEmbeddings:
  """"Lightweight word embeddings for simimilarity search"""
  def __init__(self):
    self.embeddings = {}
    self.vocabulary = set()
  ...
  def add_word(self, word: str, context: str =""):
    ...
    # Simple feature vector based on character -grams and length
  ...
  def _cosine_similarity(self, vec1: List[float], vec2: List[float]) -> float:
    ...
    magintude1 = math.sqrt(sum(a * a for a in vec1))
    magintude2 = math.sqrt(sum(a * a for a in vec2))
    
    if magintude1 == 0 or magintude2 == 0:
      return 0.0
    
    return dot_product / (magintude1 * magintude2)
...
class UserProfileManager:
    ...
    try:
      if os.path.exists(self.profile_file):
        with open(self.profile_file, 'r', encoding='utf-8') as f:
          loaded = json.load(f)
          # Merge with defaults to hadle new fields
  ...
  def record_search(self, term: str, search_type: str, found: bool):
    ...
    # Update word preferences - ensure it's defaultdict
  ...
  def record_word_interaction(self, word_type: str, difficulty: str = "meduim"):
  ...
  def get_recommendations(self) -> List[str]:
    ...
    if len(recent_searches) > 5:
      ...
      # Simple word type detection  (would be enhanced in actual word type data)
      ...
      if type_counts["complex"] > 3:
        recommendations.append("You're exploring advanced vocabulary! Try with the word type filters.")
    ...
    if self.profile["difficulty_level"] == "beginner" and self.profile["search_count"] > 10:
      recommendations.append("Ready for intermediate word? Try exploring difficulty word types.")
  ...
  def get_user_facts(self) -> Dict:
    ...
    khmer_searches = sum(1 for s in recent_searches if s.get("type") == "khmer")
    english_searches = sum(1 for s in recent_searches if s.get("type") == "english")
    ...
    return {
      "search_counts": self.profile["search_count"],
...
class AIExplanationGenerator:
  ...
  def _load_cultural_context(self) -> Dict:
    ...
    return {
      ...
      "religious_terms": "Buddhism infuences many expressions",
...
#  Related concepts
  ...
  def _get_cultural_notes(self, english: str, khmer: str, word_type: str) -> str:
    ...
    cultural_keywords = {
      ...
      "respect": ["please", "thank", "sorry", "excuse me"],
    ...
    for category, keywords in cultural_keywords.items():
      if any(keyword in english.lower() for keyword in keywords):
        if category == "family":
          return "Family terms are very specific about relationships and age."
        elif category == "respect":
          return "This word is important for polite communication in Khmmer culture."
        elif category == "religion":
          return "This terms reflect buhhist infuence in cambodia culture."
        elif category == "food":
  ...
  def _generate_usage_tips(self, english: str, khmer: str, word_type: str) -> List[str]:
    ...
    if word_type == "noun":
      tips.append("Remember: Khmer nouns don't change plural forms.")
      tips.append("Use classifiers when counting specific qualities.")
    elif word_type == "verb":
      tips.append("Add time markers (ហើយ, នឹង) to indicate tense.")
      tips.append("Verb order: Subject + Verb + Object.")
    elif word_type == "adjective":
      tips.append("Please after the noun: [noun] + [adjective].")
      tips.append("Use intensifiy with 'ណាស់' (very).") 
    elif word_type == "greeting":
      tips.append("Choose formality level based on the situation.")
      tips.append("Time of day effects which greeting to use.")   
      
      # Length-based tips 
    if len(khmer) > 6:
      tips.append("Complex word - practice writting the script. slowly")
    else :
...
class DictionaryDatabase:
  ...
  def _insert_enhanced_sample_data(self, cursor):
    """Insert sample data with enhanced features"""
    enhanced_sample_data = [
      ("hello", "សួស្តី", "greeting", "A common greeting.", "Hello, how are you?", "beginner", 10, "social,polite", "used for casual greeting"),
      ...
      ("please", "សូម", "adverb", "polite requests.", "Please help me", "beginner", 9, "polite,formal", "Make request polite"),
      ("yes", "បាទ/ចាស", "response", "Affirmative response", "Yes, I agree", "beginner", 10, "response,agreement", "បាទ for males, ចាស for females"),
      ("no", "ទេ", "response", "Negative response", "No, I don't want", "beginner", 8, "response,disagreement", "Simple negation"),
      ("water", "ទឹក", "noun", "Essential liquid", "I need water", "beginner", 9, "daily,essential", "Basic necessity"),
      ("food", "ម្ហូប", "noun", "Nutrition substance", "The food is delicious", "beginner", 8, "daily,culture", "Central to culture"),
      ("house", "ផ្ទះ", "noun", "Living building", "This my house", "beginner", 7, "home,family", "Please of residence"),
      ("school", "សាលា", "noun", "Education situation", "I go to school every day", "beginner", 6, "education,children", "Learning place"),
      ("book", "សៀវភៅ", "noun", "Written work", "I'm reading a book", "intermediate", 5, "eduction,knowledge", "Knowledge container"),
      ("student", "សិស្ស", "noun", "Learning person", "She is a good student", "intermediate", 6, "eduction,youth", "Knowledge seeker"),
      ("teacher", "គ្រូ", "noun", "Education provider", "The teacher explains well", "intermediate", 5, "eduction,respect", "Highly respect profession"),
      ("mother", "ម្ដាយ", "noun", "Female parent", "I love my mother", "beginner", 8, "family, respect", "Very importance family role"),
      ("father", "ឪពុក", "noun", "Male parent", "My father works hard", "biginner", 8, "eduction,knowledge", "Knowledge container"),
      ("beautiful", "ស្អាត", "adjective", "Pleaseing appearance", "She is beautiful", "intermediate", 6, "description,appearance", "Common compliment"),
      ("big", "ធំ", "adjective", "Large size", "This a big house", "beginner", 7, "size,description", "Basic size descriptor"),
      ("small", "តូច", "adjective", "Little size", "I have a smal car", "beginner", 7, "size,description", "Opposite of big"),
      ("eat", "ញុំា", "verb", "Consume", "Let's eat together", "beginner", 9, "dialy, action", "Essential dialy action"),
      ("go", "ទៅ", "verb", "Move to place", "I go to work", "begiinner", 9, "movement, action", "Basic movement verb")
    ]
  ...
  def create_word(self, english_word, khmer_word, word_type="noun", definition="", example="",
    ...
    try:
      ...
      ''', (english_word.lower().strip(), khmer_word.strip(), word_type, definition, example,
            difficulty, 1, cultural_tags, grammar_notes))
      conn.commit()
    ...
    try:
      if search_type == "english":
        cursor.execute('''
            ...
            WHERE english_word = ? OR english_word = ?
  ...
  def update_word(self, word_id, english_word=None, khmer_word=None, word_type=None,
                  definition=None, example=None, difficulty=None, cultural_tags=None,
                  grammar_notes=None):
  ...
  def get_random_words(self, limit=5):
    """Get random words wit enhanced data"""
    conn = sqlite3.connect(self.db_path)
    cursor = conn.cursor()
    try:
...
class DictionaryTableModel(QAbstractTableModel):
  """Enhanced table model with expert system feature"""
...
class WordDetailDialog(QDialog):
  """Enhanced dialog with AI explanation but standard UI"""
  ...
  def init_ui(self):
    ...
    if self.word_data():
      ...
      basic_info.setMaximumHeight(50)
      
      basic_content = f"""
      <h2 style='color: #2E7D32';>{english.title()} <-> {khmer}</h2>
      <p><strong>ID:</strong> {word_id}</p>
...
class TranslatorWidget(QWidget):
  ...
  def init_ui(self):
    ...
    subtitle = QLabel("វចនានុក្រមអង់គ្លេស_ខ្មែរ (ដាក់បញ្ញាសិប្បនិម្មិត)")
    ...
    self.font_manager.apply_font(suggestions_group)
    ...
    self.voice_status_label.setStyleSheet("color: #666; font_style: italic; margin-left: 5px;")
    ...
    self.random_button.clicked.connect(self.find_similar_words)
  ...
  def init_voice_search(self):
    ...
    if not SPEECH_RECOGNITION_AVAILABLE:
      ...
      self.voice_status_label.setText("Voice search requires: pip install SpeechRecogniton pyaudio")
      self.voice_status_label.setStyleSheet("color: #f44336;")
      
    else:
      self.voice_button.setEnabled(True)
      self.voice_button.setToolTip("Click and speak you search term")
  ...
  def on_voice_search_error(self, error_message):
    """Handle speak recognition error"""
  ...
  def on_voice_search_completed(self, recognized_text):
    ...
    # Update status
    self.voice_status_label.setText(f"✅ Recognized: {cleaned_text}")
  ...
  def display_no_results_with_suggestions(self, search_term, search_direction, suggestions):
    ...
    if suggestions:
      ...
      for suggestion in suggestions:
        ...
        html_content += f"""
        <div style='border-left: 3px solid #4CAF50; margin: 8px 0; padding-left: 10px;'>
  ...
  def display_expert_suggestions(self, suggestions):
    ...
    suggestions_text = " | ".join(suggestions[:2]) #Show first 2 suggestion
    self.suggestions_display.setText(f"Expert Advice: {suggestions_text}")
    ...
    if similar_words:
      ...
      for word, similarity in similar_words:
        ...
        if word_results:
          ...
          html_content += f"""
          <div style='border: 1px solid #ddd; margin: 8px 0; padding: 10px; background-color: #f9f9f9; border-radius: 5px;'>
            <strong>{english} -> {khmer}</strong> ({word_type})
  ...
  def show_smart_random_word(self):
    ...
    if filtered_words:
      ...
      self.search_input.setText(selected_word[1]) # Set English in input
...
# Enhanced Dictionary Manager with Standard UI but Expert Features
class DictionaryManagerWidget(QWidget):
  ...
  def init_ui(self):
    ...
    self.type_combo.addItems(["noun", "adjective", "adverb", "greeting", "expression", "response"])
    ...
    # Add all input groups to single horizontal layout
  ...
  def ai_assist(self):
    ...
    if not english_word:
      msg = self.font_manager.create_message_box(
        ...
        ". Use clear, simple definition\n"+
        ". Include cultural context when relevant\n" + 
        ". Choose appropriate word type\n" + 
        ". Use desciptive examples\n\n" +
  ...
  # Word type suggestions
    ...
    if suggestions:
      ...
      msg = self.font_manager.create_message_box(
        ...
        f"AI has analyzed '{english_word}' and suggestion:\n\n. {suggestion_text}\n\n" +
      ...
      msg = self.font_manager.create_message_box(
        self, QMessageBox.Icon.Information,
        "AI Assistant",
        f"No specific suggestions for '{english_word}', but here and general tips:\n\n" +
  ...
  def view_selected_word(self):
    ...
    if not selection:
      msg = self.font_manager.create_message_box(
        self, QMessageBox.Icon.Warning,
        "No Selection",
        "Please selected a word to view AI analysis!"
      )
      msg.exec()
      return
    ...
    if word_data:
      dialog = WordDetailDialog(word_data, self.font_manager, self.ai_generator, self)
      dialog.exec()
  ...
  def delete_selected_word(self):
    ...
    if not selection:
      msg = self.font_manager.create_message_box(
        self, QMessageBox.Icon.Warning,
        "No Selection",
        "Please selected a word to delete!"
      )
      msg.exec()
      return
  ...
  def filter_dictionary(self):
    ...
    for word in words:
      ...
      self.table_model.update_date(filtered_words)
...
class StatisticsWidget(QWidget):
  ...
  def init_ui(self):
    ...
    # AI insight section with scroll
  ...
  def update_stats(self):
    """Update enhanced statistics with AI insight"""
    words = self.db.read_all_words()
    ...
    # Generate AI insight words
    self.generate_ai_insights(word, user_facts)
  ...
  def generate_ai_insights(self, words, user_facts):
    ...
    if user_level == "beginner":
      recommendations.append("Focus on dialy vocabulary (food, family, greetings)")
      recommendations.append("Pactice simple sentence structures")
    ...
    # Combine insight and recommendations
    html_content = "<h4>Your Learning Insights:</h4><ul>"
    for insight in insights:
      html_content += f"<li>{insight}</lin>"
  ...
  def export_data(self):
    """Export data with compreshensive options"""
    try:
      ...
      result_text += f". Data Type: {', '.join(data_types).title()}\n"
...
# Main Application with Expert System Integration but Standard UI
class KhmerEnglishDictionaryApp(QMainWindow):
  ...
  def init_ui(self):
    ...
    self.setWindowTitle("Khmer-English Dictionary (Expert System + Voice Search) . វចនានុក្រមអងគ្លេស-ខ្មែរ")
    ...
    self.translator_tab = TranslatorWidget(self.db, self.font_manager, self.expert_engine, self.user_profile, self.ai_generator)
  ...
  def connect_signals(self):
    """Connect sigals between enhanced widget"""
    self.translator_tab.word_searched.connect(
      lambda word, search_type, found: [
        self.stats_tab.iincrement_search_count(),
    ...
    self.manager_tab.word_added.connect(
      lambda word: [
        self.statusBar().showMessage("✅ Createed word: {word}"),
    ...
    self.manager_tab.word_updated.connect(
      lambda word_id: [
        self.statusBar().showMessage(f"✅ Updated word ID: {word_id}"),
        self.stats_tab.update_stats()
      ]
    )
    ...
    self.manager_tab.word_deleted.connect(
      lambda word_id: [
        self.statusBar().showMessage(f"✅ Deleted word ID: {word_id}"),
        self.stats_tab.update_stats()
      ]
    )
...
def main():
  ...
  try:
    ...
    print(f"❌ Error starting except system application: {e}")
    import traceback
    traceback.print_exc()
